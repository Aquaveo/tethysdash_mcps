"""plugin_helpers.py — symbol-allowlist copy from tethysapp-tethys_dash.

Initial port at 2026-05-11. tethysdash_mcps owns this copy; evolution is
independent of tethysdash. Symbols retained: `get_plugin_prop`,
`DEFAULT_RUNTIME_PLACEHOLDER_GEOJSON`, `LAYER_PROPERTIES_ALLOWLIST`,
`get_allowed_source_prop_keys`, `LayerConfigurationBuilder`,
`validate_geojson`, `parse_date_hour_input`, `parse_date_input`,
`available_source_properties`. Excluded: `TethysDashPlugin` (intake.source.base
coupling), `send_websocket_message` (Django Channels coupling),
`validate_feature_collection` + `_SCAFFOLD_SHAPE_KEYS`.
"""
"""
