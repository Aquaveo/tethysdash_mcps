...
server module ``tethysdash_mcp.mcp_server`` exposes
...
def _tool_schema_properties(tool_name):
    ...
    """
    import inspect
    from tethysdash_mcp import mcp_server as srv

    fn = getattr(srv, tool_name, None)
    assert fn is not None, (
        f"{tool_name!r} not found as a module-level symbol in "
        f"tethysdash_mcp/mcp_server.py — parity contract cannot be evaluated"
    )
